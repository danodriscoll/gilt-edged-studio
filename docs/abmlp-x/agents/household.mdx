...
def tax_strategy(self, tax_to_pay):
    """ Tax calculated from wage supplied, current step.

    Receives a 'tax_to_pay' amount calculated by either,
    flat_tax_to_pay() or marginal_tax_to_pay().

    To develop..
    """
    tax_payment = Decimal(0)
    tax_payment = tax_to_pay * Decimal(1) # Pay 100% of tax owed.
