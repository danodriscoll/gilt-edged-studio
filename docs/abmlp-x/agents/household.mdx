...
def tax_strategy(self, amount):
    """ Final tax amount paid (wage income).
    Receives amount calculated by either,
    'flat_tax_to_pay()', or marginal_tax_to_pay().

    To be developed..
    """
    final_tax_paid = Decimal(0)
    final_tax_paid = amount * Decimal(1) # Pay all tax owed.

    return final_tax_paid
```
